...
SequenceMining::SequenceMining(const std::string& filename, int minsup, int maxpat) {
    this->minsup = minsup;
    this->maxpat = maxpat;
...
void SequenceMining::mining(vecVec& Xt, PatternSupportList& patternSupportList) {
    ProjectDB pdb;
    for (int i = 0; i < n; ++i)
...
void SequenceMining::__mining(const ProjectDB& pdb, vecVec& Xt, Pattern& pattern, std::vector<int> &supportList, PatternSupportList& patternSupportList) {
    ...
    for (const auto& it : counter) {
        ...
        Xt.push_back(x);
        patternSupportList.push_back(pair<Pattern, int>(pattern, support));
...
tree<SequenceMining::Save>::iterator SequenceMining::createRoot(){
    if (Tree.empty()){
        ProjectDB pdb;
        for (int i = 0; i < n; ++i)
            pdb.push_back(pair<int, int>(i, -1));
...
void SequenceMining::createChildren(const tree<Save>::iterator &node){
    if (!node->nextCheck){
        node->nextCheck=true;
...
double SequenceMining::getMaxValue(const std::vector<double>& v)  {
    visit=0;
    double maxval = 0;
...
void SequenceMining::__getMaxValue(const tree<Save>::iterator &node, double& maxval, const std::vector<double>& v){
    ++visit;
...
void SequenceMining::safePatternPruning(double r, const std::vector<double>& theta, vecVec& Xt, PatternSupportList& patternSupportList) {
    Xt.clear();
    patternSupportList.clear();
    visit=0;
...
void SequenceMining::__safePatternPruning(const tree<Save>::iterator &node, double r, const std::vector<double>& theta, vecVec& Xt, PatternSupportList& patternSupportList) {
    ++visit;
...
void SequenceMining::regularizationPath(int splitNum, double R, int maxloop, double eps, int freq) {
